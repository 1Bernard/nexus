defmodule NexusWeb.Admin.AnomalyInvestigationLive do
  ...
  def mount(%{"id" => id}, _session, socket) do
    ...
    # In a full system, we might query the exact invoice by source_id/invoice_id from ERP.
    # For now, we simulate fetching the raw invoice details to show the split-screen power.
    # We use ERP.get_invoice! if it existed, but we'll mock the line items based on anomaly data.

    raw_invoice = %{
      id: anomaly.invoice_id,
      # Ideally fetched from invoice projection
      vendor: "CorpTech",
      date: NaiveDateTime.to_date(anomaly.inserted_at),
      total_amount: "€4,250.00",
      line_items: [
        %{desc: "Cloud Hosting - Compute", qty: 1, price: "€1,200.00"},
        %{desc: "Cloud Hosting - Storage", qty: 1, price: "€550.00"},
        # The anomaly!
  ...
  def render(assigns) do
    ...
    <.page_container>
      <.page_header title={@page_title} subtitle={@page_subtitle}>
        <:actions>
          <.link navigate={~p"/admin/analysis"} class="flex items-center gap-2 text-xs font-bold text-slate-400 hover:text-white transition-colors uppercase tracking-widest">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="space-y-6">
          <.dark_card class="p-8 border-rose-500/20 shadow-[0_0_30px_rgba(244,63,94,0.05)]">
            ...
            <div class="space-y-6">
              <div>
                <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Detection Reason</label>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="p-4 bg-slate-900/50 rounded-xl border border-white/5">
                  <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Context Org</label>
                  <span class="text-slate-300 text-xs font-mono">{String.slice(@anomaly.org_id, 0, 8)}</span>
                </div>
                <div class="p-4 bg-slate-900/50 rounded-xl border border-white/5">
                  <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Flagged At</label>
                  <span class="text-slate-300 text-xs font-mono">{Calendar.strftime(@anomaly.flagged_at, "%Y-%m-%d %H:%M:%S UTC")}</span>
                </div>
              </div>
            </div>
          </.dark_card>
          ...
          <!-- Resolution Actions -->
          <.dark_card class="p-8">
            <h3 class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-6 border-b border-white/5 pb-4">Required Action</h3>
            <div class="flex gap-4">
              ...
              >
                <.icon name="hero-check-circle" class="w-8 h-8 text-emerald-500 group-hover:scale-110 transition-transform" />
                <span class="text-xs font-bold text-emerald-400 uppercase tracking-widest mt-2">Clear Anomaly</span>
                <span class="text-[10px] text-emerald-500/70 text-center px-4">Mathematical deviation justified. Approve for payment flow.</span>
              </button>
              ...
              >
                <.icon name="hero-shield-exclamation" class="w-8 h-8 text-rose-500 group-hover:scale-110 transition-transform" />
                <span class="text-xs font-bold text-rose-400 uppercase tracking-widest mt-2">Mark as Fraud</span>
                <span class="text-[10px] text-rose-500/70 text-center px-4">Suspend payment and alert audit continuity team immediately.</span>
              </button>
            </div>
          </.dark_card>
        </div>
          ...
          <.dark_card class="p-0 h-full flex flex-col">
            <div class="p-8 border-b border-white/5">
              <h2 class="text-sm font-bold text-slate-300 uppercase tracking-widest flex items-center gap-3">
                <.icon name="hero-document-text" class="w-5 h-5 text-slate-500" /> Source Invoice Context
              </h2>
            </div>
            ...
            <div class="p-8 flex-1">
              <div class="flex justify-between items-start mb-10">
                <div>
                  <h1 class="text-2xl font-black text-white tracking-tight">{@raw_invoice.vendor}</h1>
