defmodule NexusWeb.Tenant.AnomalyInvestigationLive do
  @moduledoc """
  LiveView for tenant users to investigate and resolve detected anomalies.
  """
  ...
  def mount(%{"id" => id}, _session, socket) do
    ...
    socket =
      socket
      |> assign(:page_title, "Analysis Deep-Dive")
      ...
      :ok ->
        msg =
          if resolution == "cleared",
            do: "Anomaly mathematically cleared. Processing resumed.",
            else: "Issue flagged for fraud review. Compliance team notified."

        {:noreply,
         socket
         |> put_flash(:info, msg)
         |> push_navigate(to: ~p"/intelligence")}

      {:error, reason} ->
        {:noreply, put_flash(socket, :error, "Resolution failed: #{inspect(reason)}")}
    end
  end
    ...
    <.page_container>
      <.page_header title={@page_title} subtitle={@page_subtitle}>
        <:actions>
          ...
          >
            <.icon name="hero-arrow-left" class="w-4 h-4" /> Back to Insights
          </.link>
        </:actions>
      </.page_header>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="space-y-6">
          <.dark_card class="p-8 border-indigo-500/20 shadow-[0_0_30px_rgba(99,102,241,0.05)]">
            <div class="flex items-center justify-between mb-8">
              <h2 class="text-sm font-bold text-indigo-400 uppercase tracking-widest flex items-center gap-3">
                <.icon name="hero-cpu-chip" class="w-5 h-5" /> Inference Engine Output
              </h2>
              <div class="px-3 py-1 bg-indigo-500/10 rounded border border-indigo-500/20 text-indigo-400 text-xs font-mono font-bold">
            ...
            <div class="space-y-6">
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="p-4 bg-slate-900/50 rounded-xl border border-white/5">
                  ...
                  <span class="text-slate-300 text-xs font-mono">ERP Stream // Invoice</span>
                </div>
                <div class="p-4 bg-slate-900/50 rounded-xl border border-white/5">
                  <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">
                    Flagged At
            ...
            <div class="flex gap-4">
              ...
              >
                ...
                <span class="text-[10px] text-emerald-500/70 text-center px-4">
                  Approved following forensic review. Resume accounting cycle.
                </span>
              </button>
              ...
              >
                ...
                <span class="text-[10px] text-rose-500/70 text-center px-4">
                  Block settlement and trigger compliance audit immediately.
                </span>
              </button>
            </div>
          </.dark_card>
          ...
          <.dark_card class="p-0 h-full flex flex-col min-h-[500px]">
            <div class="p-8 border-b border-white/5">
              <h2 class="text-sm font-bold text-slate-300 uppercase tracking-widest flex items-center gap-3">
                <.icon name="hero-document-text" class="w-5 h-5 text-slate-500" /> Source Context
              </h2>
            </div>
