defmodule Nexus.Treasury.ReconciliationTest do
  ...
  defgiven ~r/^the following invoices exist:$/, %{table: table}, state do
    ...
    mapping =
      Enum.reduce(table, %{}, fn row, accumulator ->
        uuid = Nexus.Schema.generate_uuidv7()
        Map.put(accumulator, row.invoice_id, uuid)
      end)
    ...
    Enum.each(table, fn row ->
      ...
      command = %IngestInvoice{
        ...
        amount: Nexus.Schema.parse_decimal(row.amount),
        subsidiary: "Default",
        due_date: Date.utc_today() |> Date.add(30),
        line_items: [%{description: "Test", amount: Nexus.Schema.parse_decimal(row.amount)}],
      ...
      {:ok, [%{data: event, event_number: event_number}]} =
        Nexus.EventStore.read_stream_forward(invoice_id)

      project_event(
        event,
        event_number,
        "ERP.InvoiceProjector",
        Nexus.ERP.Projectors.InvoiceProjector
      )

      sync_reconciliation_manager(event, event_number, state)
    end)
  ...
  defgiven ~r/^the following statement lines exist:$/, %{table: table}, state do
    ...
    line_mapping =
      Ecto.Adapters.SQL.Sandbox.unboxed_run(Nexus.Repo, fn ->
        ...
        Repo.all(
          from line in Nexus.ERP.Projections.StatementLine,
            where: line.statement_id == ^statement_id
        )
        |> Enum.into(%{}, fn line -> {String.replace(line.ref, "REF-", ""), line.id} end)
      end)
  ...
  defgiven ~r/^an invoice "(?<id>[^"]+)" is ingested for (?<amount>[^ ]+) (?<currency>.+)$/,
    ...
    command = %IngestInvoice{
      ...
      currency: currency,
      amount: Nexus.Schema.parse_decimal(amount),
      subsidiary: "Default",
      due_date: Date.utc_today() |> Date.add(30),
      line_items: [%{description: "Test", amount: Nexus.Schema.parse_decimal(amount)}],
  ...
  defthen ~r/^the invoice "(?<invoice_id_alias>[^"]+)" and statement line "(?<line_id_alias>[^"]+)" should be reconciled$/,
          %{invoice_id_alias: invoice_id_alias, line_id_alias: line_id_alias},
          state do
    invoice_id = Map.get(state.invoice_mapping, invoice_id_alias)
    statement_line_id = Map.get(state.line_mapping, line_id_alias)

    reconciliation = get_reconciliation_by_invoice(invoice_id)

    assert reconciliation != nil,
           "Expected reconciliation record for invoice #{invoice_id_alias} (#{invoice_id})"

    assert reconciliation.status == :matched
    assert reconciliation.statement_line_id == statement_line_id
    {:ok, state}
  end
  ...
  defthen ~r/^a settlement unmatched exception should be recorded for "(?<line_id_alias>[^"]+)"$/,
          ...
          state do
    statement_line_id = Map.get(state.line_mapping, line_id_alias)
    line = get_statement_line(statement_line_id)
    assert line != nil, "Statement line #{line_id_alias} not found"
    reconciliation = get_reconciliation_by_statement_line(statement_line_id)
    assert reconciliation == nil
    {:ok, state}
  end

  defthen ~r/^the process manager should dispatch a ReconcileTransaction command for "(?<invoice_id_alias>[^"]+)"$/,
          %{invoice_id_alias: invoice_id_alias},
          state do
    invoice_id = Map.get(state.invoice_mapping, invoice_id_alias)
    reconciliation = get_reconciliation_by_invoice(invoice_id)
    assert reconciliation != nil
  ...
  defp sync_reconciliation_manager(event, event_number, state) do
    ...
    pm_state =
      case Nexus.EventStore.read_stream_forward(state.pm_stream) do
        {:ok, events} ->
          Enum.reduce(events, %ReconciliationManager{}, fn %{data: event}, accumulator ->
            ReconciliationManager.apply(accumulator, event)
          end)
  ...
  defp dispatch_and_project_recon(command) do
    ...
    {:ok, [%{data: event, event_number: event_number}]} =
      Nexus.EventStore.read_stream_forward(reconciliation_id)
  ...
  defp get_reconciliation_by_invoice(invoice_id) do
    Ecto.Adapters.SQL.Sandbox.unboxed_run(Nexus.Repo, fn ->
      ...
      Repo.one(
        from reconciliation in Reconciliation, where: reconciliation.invoice_id == ^invoice_id
      )
    end)
  end

  defp get_reconciliation_by_statement_line(statement_line_id) do
    Ecto.Adapters.SQL.Sandbox.unboxed_run(Nexus.Repo, fn ->
      import Ecto.Query

      Repo.one(
        from reconciliation in Reconciliation,
          where: reconciliation.statement_line_id == ^statement_line_id
      )
    end)
  end

  defp get_statement_line(statement_line_id) do
    Ecto.Adapters.SQL.Sandbox.unboxed_run(Nexus.Repo, fn ->
      import Ecto.Query

      Repo.one(
        from line in Nexus.ERP.Projections.StatementLine, where: line.id == ^statement_line_id
      )
    end)
  end
end
