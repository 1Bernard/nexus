defmodule Nexus.Treasury.PolicyModeTest do
  ...
  defgiven ~r/^the organization "(?<name>[^"]+)" is on "(?<mode>[^"]+)" mode$/,
    ...
    assert Enum.any?(events, fn e -> e.data.__struct__ == PolicyModeChanged end)
  ...
  defwhen ~r/^the system evaluates an exposure of "(?<amount>[^"]+)" EUR\/USD$/,
    ...
    assert :ok == App.dispatch(threshold_cmd)
    ...
    assert :ok == App.dispatch(evaluate_cmd)

    # Project all events in stream
    {:ok, events} = Nexus.EventStore.read_stream_forward(state.org_id)
    Enum.each(events, fn e -> project_event(e.data, e.event_number) end)
