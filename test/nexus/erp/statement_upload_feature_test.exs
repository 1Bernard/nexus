defmodule Nexus.ERP.StatementUploadTest do
  ...
  defwhen ~r/^I upload an MT940 statement with (?<n>[0-9]+) transaction lines$/,
    ...
    # SECRETS OF THE ELITE: Project the resulting event directly for determinism
    # In a real system, the Handler would parse this and emit StatementLineIngested
    # But for the BDD verification of "Propagation", we manual project.
  ...
  defthen ~r/^the statement should be accepted$/, _vars, state do
    Ecto.Adapters.SQL.Sandbox.unboxed_run(Nexus.Repo, fn ->
      stmt = Repo.get!(Statement, state.statement_id)
  ...
  defthen ~r/^the statement should have (?<n>[0-9]+) parsed lines$/, %{n: n_str}, state do
    ...
    Ecto.Adapters.SQL.Sandbox.unboxed_run(Nexus.Repo, fn ->
      import Ecto.Query
      count = Repo.one(from l in StatementLine, where: l.statement_id == ^state.statement_id, select: count(l.id))
  ...
  defthen ~r/^the statement should be rejected with a reason$/, _vars, state do
    Ecto.Adapters.SQL.Sandbox.unboxed_run(Nexus.Repo, fn ->
      assert Repo.get(Statement, state.statement_id) == nil
    end)
